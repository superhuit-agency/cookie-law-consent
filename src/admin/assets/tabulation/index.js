...
export default class Tabulation {
	...
	onTabClick(event) {
		...
		this.updateActive(
			Array.from(this.refs.tabList.children).indexOf(event.target)
		);
	}

	updateActive(nextActive = 0) {
		this.refs.tabList.children[this.state.activeTab].classList.remove(
			"is-active"
		);
		this.refs.panelList.children[this.state.activeTab].classList.remove(
			"is-active"
		);
