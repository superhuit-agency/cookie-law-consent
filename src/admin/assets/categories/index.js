...
export default class Categories {
	constructor(element) {
		...
		this.refs = {
			...
			categories: Array.from(this.el.querySelectorAll(".category")).map(
				(cat) => new Category(cat, this.props.settingName)
			),
		};
	...
	bind() {
		...
		this.refs.categories.forEach((cat) =>
			cat.el.addEventListener("titleChange", this.onCategoryChange)
		);
	}
	...
	onCategoryChange(event) {
		const catIdx = this.refs.categories.findIndex(
			(cat) => cat.getId() === event.detail.id
		);
		this.refs.tabList.children[catIdx].textContent =
			event.detail.title === "" ? `Category ${catIdx + 1}` : event.detail.title;
	}
	...
	renderTabs() {
		...
		this.refs.categories.forEach((cat, id) => {
			...
			this.refs.tabList.insertAdjacentHTML(
				"beforeend",
				`<a href="#category_${cat.getId()}" class="categories__tab${
					this.state.activeId === id ? " is-active" : ""
				}">${title === "" ? `Category ${id + 1}` : title}</a>`
			);
		});
	}
}
