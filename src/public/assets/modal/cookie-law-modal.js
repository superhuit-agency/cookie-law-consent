...
export default class CookieLawModal extends EventEmitter {
	...
	bindEvents() {
		...
		this.refs.categories.forEach((cat) =>
			cat.on("change", this.onCategoryChange)
		);
	}
	...
	unbindEvents() {
		...
		this.refs.categories.forEach((cat) =>
			cat.off("change", this.onCategoryChange)
		);
	}
	...
	init() {
		...
		this.refs.el.insertAdjacentHTML(
			"beforeend",
		...
		this.refs.list = this.refs.el.querySelector(
		...
		this.refs.categories = this.config.categories.map(
			(cat) => new CookieLawCategory(this.refs.list, cat)
		);
