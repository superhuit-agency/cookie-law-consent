...
export default class CookieLawCategory extends EventEmitter {
	constructor(container, config) {
		...
		if (this.refs.checkbox)
			this.refs.checkbox.addEventListener("change", this.onChange);
		if (this.refs.switch)
			this.refs.switch.addEventListener("click", this.stopPropagation);
	}

	destroy() {
		this.refs.tab.removeEventListener("click", this.onToggle);
		if (this.refs.checkbox)
			this.refs.checkbox.removeEventListener("change", this.onChange);
		if (this.refs.switch)
			this.refs.switch.removeEventListener("click", this.stopPropagation);
	}
			...
			`<div class="cookie-law-category__tab">
				...
				${
					this.config.mandatory
						...
						>
						<label class="cookie-law-category__label" for="cookie-law-category-checkbox-${
							this.config.id
						}">
	...
	toggleExpanded() {
		...
		this.refs.el.classList[this.state.collapsed ? "remove" : "add"](
			CLASS_IS_EXPANDED
		);
	}
	...
	toggleStatus() {
		...
		if (
			this.refs.checkbox &&
			this.state.enabled !== this.refs.checkbox.checked
		) {
